...
def add_file_prefix(df, file_name):
    """Add a prefix to each column name to indicate the source file."""
    df.columns = [f"{file_name}_{col}" if col != 'PATNO' and col != 'EVENT_ID' else col for col in df.columns]
    return df

def merge_csv_files(file_list, output_file):
    """Merges CSV files and maintains column order with file-specific prefixes."""
    ...
    merged_df = pd.read_csv(first_file)
    merged_df = add_file_prefix(merged_df, os.path.basename(first_file).replace('.csv', ''))

    # Read and merge the remaining files
    for file_path in file_list[1:]:
        df = pd.read_csv(file_path)
        file_prefix = os.path.basename(file_path).replace('.csv', '')
        df = add_file_prefix(df, file_prefix)
        
        # Merge the new DataFrame with the accumulated DataFrame
...
output_file = 'merged_files_with_origin.csv'
